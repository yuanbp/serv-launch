...
//    }
    ...
    public AjaxResult editPassword(HttpSession session,String password,String newpassword,String repassword){
        ...
        SysUser admin1= userService.selectById(admin.getId());
    ...
    public String info(HttpSession session, Model model){
        SysUser admin = (SysUser) session.getAttribute(Const.ADMIN);
        SysRole role = roleService.selectById(admin.getRid());
    ...
    @ResponseBody
    public AjaxResult editInfo(SysUser admin,HttpSession session){
        SysUser ad = (SysUser) session.getAttribute(Const.ADMIN);
        ad.setUsername(admin.getUsername());
        ad.setEmail(admin.getEmail());
        ad.setPhone(admin.getPhone());
        int count = userService.editBySysUser(ad);
        if(count >= 1){
